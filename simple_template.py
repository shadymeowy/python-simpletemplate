...
class SimpleTemplate(BaseTemplate):
    def prepare(self,
        ...
        self.cache = {}
        enc = self.encoding
        self._str = lambda x: x
        self._escape = lambda x: escape_func(x)
        self.syntax = syntax
        if noescape:
    ...
    def _include(self, _env, _name=None, **kwargs):
        ...
        if _name not in self.cache:
            self.cache[_name] = self.__class__(name=_name, syntax=self.syntax)
...
class StplParser(object):
    ...
    def __init__(self, source, syntax=None, encoding='utf8'):
        self.source, self.encoding = source, encoding
...
def template(*args, **kwargs):
    ...
    if tplid not in TEMPLATES or DEBUG:
        ...
        if isinstance(tpl, adapter):
            ...
            TEMPLATES[tplid] = adapter(source=tpl, **settings)
        else:
